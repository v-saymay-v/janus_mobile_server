...
function rotateImage($b64) {
	$image_resource = imagecreatefromstring(base64_decode($b64));
	$sx = imagesx($image_resource);
	$sy = imagesy($image_resource);
	if ($sy > $sx) {
		$rotate = imagerotate($image_resource, 90, 0);
		ob_start();
		imagejpeg($rotate);
...
function getCardImg($photo) {
	if (empty($photo)) {
		if (isset($smart_phone)) {
			return '';
		} else {
			return '<img alt="Embedded Image" width=360 src="common/images/blank.png" />';
		}
	}
	$b64 = explode(";", $photo);
	if(count($b64) == 1) {
		...
		$b64 = rotateImage($b64);
		return ($photo != "" ? '<img alt="Embedded Image" width=360 src="data:image/jpg;base64,'.$b64.'" />' : "");
	} else if(count($b64) >= 3) {
		$b64 = $b64[2];
		$b64 = explode(":", $b64);
		if(count($b64) >= 2) {
...
function showOneEntry($r, $only_phone = false) {
	...
	global $db, $table, $table_grp_adr, $table_groups, $print, $is_fix_group, $mail_as_image, $page_ext_qry, $smart_phone;

	$view = "";
	if (!empty($r['fullname'])) {
		$view .= add("<b>".$r['fullname']."</b>");
	} else {
		$view .= add("<b>".$r['lastname'].(!empty($r['middlename']) ? " ".$r['middlename'] : "")." ".$r['firstname']."</b>");
	}
  //$view .= add($r['nickname']);

  $view .= getCardImg(str_replace(" ", "", $r['photo'])) . "&nbsp;&nbsp;";
	$view .= getCardImg(str_replace(" ", "", $r['back'])) . "<br />";
/*
  $b64 = explode(";", $r['photo']);
    ...
    if(count($b64) >= 2) {
      $b64 = str_replace(" ", "", $b64[1]);
      $view .= ($r['photo'] != "" ? '<img alt="Embedded Image" width=240 src="data:image/jpg;base64,'.$b64.'"/><br>' : "");
    }
  }
*/
  if(! $only_phone) {
...
//     $view .= addGroup($r, array('address'));
  ...
  if(! $only_phone) {
    ...
    if($mail_as_image) { // B64IMG: Thanks to NelloD
	  ...
	  $view .= addGroup($r, array('bday','bmonth_num','byear'));
	  $view .= addBirthday($r['bday'], $r['bmonth_num'], $r['byear'], ucfmsg('BIRTHDAY'));
