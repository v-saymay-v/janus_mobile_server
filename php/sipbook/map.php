...
<?php include ("include/header.inc.php");
  ...
  //$base_url = "https://maps.google.ch/maps/geo?output=csv&key=".$google_maps_key;
  $first_fetch = true;
  ...
  while($myrow = mysqli_fetch_array($result)) {
    ...
    //if(!($coord['status'] == 200 || $coord['status'] == 602 )) {
		if(!empty($coord['addr'])) {
			$url = "https://maps.google.com/maps/api/geocode/json?address=".urlencode($coord['addr'])."&key=".$google_maps_key;
			...
			if ($response->status == 'OK') {
			    $coord['lati'] = $response->results[0]->geometry->location->lat;
			    $coord['long'] = $response->results[0]->geometry->location->lng;
...
*/
  ...
  ?>
    <script src="http://maps.google.com/maps?file=api&amp;v=2&amp;sensor=true&amp;key=<?php echo $google_maps_key; ?>" type="text/javascript"></script>
