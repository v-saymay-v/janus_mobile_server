...
$(document).ready(function() {
	...
	var unregisteredcb = null;
	var sipcallsetup = null;
		...
		janus = new Janus({
			...
			success: function() {
				...
				janus.attach({
					...
					success: function(pluginHandle) {
						...
						$('.registerset').unbind('click').click(function() {
							selectedApproach = $(this).val();
							if(selectedApproach === "guest") {
								$('#sipassword').empty().attr('disabled', true);
							} else {
								$('#sipassword').removeAttr('disabled');
							}
							return true;
						});
					...
					onmessage: function(msg, jsep) {
						...
						if(error) {
							if(!registered) {
								...
								$('#sipdispname').removeAttr('disabled');
								$('#sipassword').removeAttr('disabled');
								...
								$('#sipcall').removeAttr('disabled').html('Call')
						...
						if(result && result["event"]) {
							...
							if(event === 'registration_failed') {
								...
								$('#sipassword').removeAttr('disabled');
								$('.registerset').removeAttr('disabled');
								bootboxalert(result["code"] + " " + result["reason"]);
								if (registeredcb)
									registeredcb(result["reason"]);
								return;
							}
							if(event === 'registered') {
								...
								if(!registered) {
									...
									$('#sipdispname').parent().addClass('hide').hide();
									$('#sipassword').parent().addClass('hide').hide();
									...
									$('#sipcall').unbind('click').click(doCall);
									$('#peer').focus();
									*/
									if (registeredcb)
										registeredcb();
								...
								// TODO Any ringtone?
								$('#sipcall').removeAttr('disabled').html('Hangup')
			...
			error: function(error) {
				...
				bootboxalert(error, async function() {
					await jumpToLocation('index.php');
				});
			},
	...
	window.sipRegisterUsername = function(finishedcb) {
		if(!selectedApproach) {
			bootboxalert("Please select a registration approach from the dropdown menu");
			return;
		}
		if (registered) {
			sipcall.send({ message: { "request" : "unregister" }, success: function() {
				unregisterdcb = function() {
					registerUsername(finishedcb);
				}
			} });
		} else {
			registerUsername(finishedcb);
	...
	window.sipDestroy = function() {
		if (janus) {
			janus.destroy();
		}
	}

	function registerUsername(finishedcb) {
		// Try a registration
		$('#server').attr('disabled', true);
		$('#username').attr('disabled', true);
		$('#authuser').attr('disabled', true);
		$('#sipdispname').attr('disabled', true);
		$('#sipassword').attr('disabled', true);
		...
		if(sipserver !== "" && sipserver.indexOf("sip:") != 0 && sipserver.indexOf("sips:") !=0) {
			bootboxalert("Please insert a valid SIP server (e.g., sip:192.168.0.1:5060)");
			$('#server').removeAttr('disabled');
			$('#username').removeAttr('disabled');
			$('#authuser').removeAttr('disabled');
			$('#sipdispname').removeAttr('disabled');
		...
		window.sipLogin($('#server').val(), $('#username').val(), $('#authuser').val(), $('#sipdispname').val(), $('#sipassword').val(), selectedApproach,
			function() {
				registeredcb = finishedcb;
			},
			function(error) {
				if (error) {
					console.log(error);
		...
		if(selectedApproach === "guest") {
			...
			var displayname = $('#sipdispname').val();
			if(displayname) {
			...
			if(sipserver === "") {
				bootbox.confirm("You didn't specify a SIP Registrar to use: this will cause the plugin to try and conduct a standard (<a href='https://tools.ietf.org/html/rfc3263' target='_blank'>RFC3263</a>) lookup. If this is not what you want or you don't know what this means, hit Cancel and provide a SIP Registrar instead'",
					function(result) {
						if(result) {
							sipcall.send({ message: register, success: registerSipSetting });
						} else {
							$('#server').removeAttr('disabled');
							$('#username').removeAttr('disabled');
							$('#authuser').removeAttr('disabled');
	...
	window.sipLogin = function(server, username, authuser, displayname, password, registerset, successcb, failedcb) {
		selectedApproach = registerset;
		if(selectedApproach === "guest") {
			// We're registering as guests, no username/secret provided
			var register = {
				request: "register",
			...
			if(!username === "" || username.indexOf("sip:") != 0 || username.indexOf("@") < 0) {
				if (failedcb)
					failedcb("Please insert a valid SIP address (e.g., sip:goofy@example.com): this doesn't need to exist for guests, but is required");
				return;
			}
			register.username = username;
			if(displayname) {
			...
			if(server === "") {
				bootbox.confirm("You didn't specify a SIP Registrar to use: this will cause the plugin to try and conduct a standard (<a href='https://tools.ietf.org/html/rfc3263' target='_blank'>RFC3263</a>) lookup. If this is not what you want or you don't know what this means, hit Cancel and provide a SIP Registrar instead'",
					function(result) {
						if(result) {
							sipcall.send({ message: register, success: successcb?successcb:function(){} });
						} else {
							if (failedcb)
								failedcb();
						}
		...
		if(username === "" || username.indexOf("sip:") != 0 || username.indexOf("@") < 0) {
			if (failedcb)
				failedcb('Please insert a valid SIP identity address (e.g., sip:goofy@example.com)');
			return;
		}
		if(password === "") {
			if (failedcb)
				failedcb("Insert the username secret (e.g., mypassword)");
			return;
		}
		var register = {
			request: "register",
	...
	window.sipDoCall = function(ev) {
		...
		$('#sipcall' + suffix).attr('disabled', true).unbind('click');
		$('#dovideo' + suffix).attr('disabled', true);
		var username = $('#peer' + suffix).val();
		if(username === "") {
			bootboxalert('Please insert a valid SIP address (e.g., sip:pluto@example.com)');
			$('#peer' + suffix).removeAttr('disabled');
			$('#dovideo' + suffix).removeAttr('disabled');
			$('#sipcall' + suffix).removeAttr('disabled').click(function() { doCall(helperId); });
			return;
		}
	...
	window.sipDoHangup = function(ev) {
		...
		if(helperId === "")
			...
			helperId = parseInt(helperId);
		if(!helperId) {
			$('#sipcall').attr('disabled', true).unbind('click');
			var hangup = { request: "hangup" };
			sipcall.send({ message: hangup });
			sipcall.hangup();
		} else {
			$('#sipcall' + helperId).attr('disabled', true).unbind('click');
			var hangup = { request: "hangup" };
	...
	window.sipAddHelper = function(helperCreated) {
		helpersCount++;
			...
			onmessage: function(msg, jsep) {
				...
				if(result && result["event"]) {
					...
					if(event === 'registered') {
						...
						$('#sipcall' + helperId).removeAttr('disabled').html('Call')
							.removeClass("btn-danger").addClass("btn-success")
							.unbind('click').click(doCall);
						if(helperCreated && typeof helperCreated === 'function')
							helperCreated(helperId);
					} else if(event === 'calling') {
						Janus.log("[Helper #" + helperId + "] Waiting for the peer to answer...");
						// TODO Any ringtone?
						$('#sipcall' + helperId).removeAttr('disabled').html('Hangup')
							  .removeClass("btn-success").addClass("btn-danger")
							  .unbind('click').click(doHangup);
					} else if(event === 'incomingcall') {
	...
	window.setSipcallSetupCB = function(setupcb) {
		sipcallsetup = setupcb;
	}

	function jumpToLocation(jumpto) {
