...
if ($stmt) {
  ...
  $stmt->execute();
  $stmt->bind_result($tag, $fromId, $toId, $fromName, $toName, $create, $answer);
  $calls = array();
  while ($stmt->fetch()) {
	$name = ($fromId==$app->loggedInUser->user_id?$toName:$fromName);
	$isAnswered = empty($answer)?'未':'済';
	$make = ($fromId==$app->loggedInUser->user_id?'発信':'着信');

    $call = array("tag" => $tag, "make" => $make, "name" => $name, "create" => $create, "answered" => $isAnswered);
    $calls[] = $call;
  }
  $stmt->close();
} else {
...
$data = array("result" => 0, "calls" => $calls);
