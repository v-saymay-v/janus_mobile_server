...
$(document).ready(function() {
			...
			success: function() {
				...
				janus.attach({
					...
					success: function(pluginHandle) {
						...
						$('#record').removeAttr('disabled').html("録音")
							.click(function() {
								$(this).attr('disabled', true);
								startRecording();
							});
								...
								if(msg["status"]) {
									...
									if(status === 'starting') {
										...
										$('#record')
											...
											.text("録音中");
										$('#recordmess').addClass('hide').hide();
									} else if(status === 'done') {
										$('#record')
											.removeClass("btn-info").addClass("btn-success")
											.text("完了");
										$('#downloadvoicemail').attr('href', './playvoicemail.php?tag='+$('#downloadvoicemail').attr('name')/*msg["recording"]*/);
										$('.listenvoicemail').click(function() {
											const thiz = this;
											$('#rec').remove();
											$(parent()).parent().append(
												'<audio id="rec" style="width:100%;height:100%;" autoplay controls preload="auto">' +
													'<source id="opusrec" src="./playvoicemail.php?tag=' + $(thiz).attr('name')/*msg["recording"]*/ + '" type="audio/ogg""></source>' +
													'Your browser doesn\'t support the playout of Opus files' +
												'</audio>'
											);
											$('#opusrec').attr('type', 'audio/ogg; codecs="opus"');
											if($('#opusrec').get(0).error) {
												bootboxalert("Couldn't play the Opus recording (" + $('#opusrec').get(0).error + "), try downloading it instead");
											}
											return false;
										});
										$('#playdone').removeClass('hide').show();
										vmailtest.hangup();
										if (voiceMailRecorded)
											voiceMailRecorded(msg["recording"]);
									}
								} else if(msg["error"]) {
									bootboxalert(msg["error"], function() {
	...
	window.destroyVoiceMail = function() {
		if (janus) {
			janus.destroy();
		}
	}
