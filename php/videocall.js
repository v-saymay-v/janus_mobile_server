...
var attachedcb = null;
var gotuserlistcb = null;
var acceptcallcb = null;
var incommingCallDialog = null;
var openCallingDialog = null;
var closeCallingDialog = null;

var fromUser = getQueryStringValue("from");
var toUser = getQueryStringValue("to");
...
//var waitRegisterUsername = null;
//var lastUserList = null;	//new Array();
				...
				janus.attach({
					...
					onmessage: function(msg, jsep) {
						...
						if(result) {
							if(result["list"]) {
								...
								for(var mp in list) {
									Janus.debug("  >> [" + list[mp] + "]");
									var nm = decodeURI(list[mp]);
									users.push(nm.replace('+', ' '));
								}
								//var differ = false;
								//if (list.length != lastUserList.length || !array_equal(list, lastUserList)) {
								//	differ = true;
								//}
								//if (differ && gotuserlistcb) {
									gotuserlistcb(users);
								//}
								//if (waitRegisterUsername) {
								//	realRegisterUsername(waitRegisterUsername);
								//	waitRegisterUsername = null;
								//}
								//lastUserList = list;
								...
								if(event === 'registered') {
									...
									var decoded = decodeURI(myusername);
									var idx = decoded.indexOf('_');
									var myname = decoded.substring(idx+1);
									$('#youok').removeClass('hide').show().html("Registered as '" + myname + "'");
									...
									// TODO Any ringtone?
									if (openCallingDialog) {
										openCallingDialog(function() {
											videocall.hangup();
										});
									} else {
										bootboxalert("Waiting for the peer to answer...");
									}
									if ($('#ringbacktone').length > 0) {
										//$('#ringbacktone')[0].play();
									}
									...
									var decoded = decodeURI(yourusername);
									var idx = decoded.indexOf('_');
									var yourid = decoded.substring(0, idx);
									var yourname = decoded.substring(idx+1);
									// Notify user
									if (incommingCallDialog) {
										incommingCallDialog(yourname, function() {
											incoming = null;
											if ($('#ringtone').length > 0) {
												$('#ringtone')[0].pause();
											}
										...
										incoming = bootbox.dialog({
											message: "Incoming call from " + yourname + "!",
											title: "Incoming call",
											closeButton: false,
											buttons: {
												success: {
					...
					onremotestream: function(stream) {
						...
						if($('#remotevideo').length === 0) {
							...
							var idx = decoded.indexOf('_');
							var yourname = decoded.substring(idx+1);
							$('#callee').removeClass('hide').html(decodeURI(yourname)).show();
						}
...
function realRegisterUsername(username) {
	for (var mp in lastUserList) {
		if (lastUserList[mp] == encodeURI(username)) {
			return;
		}
	}
	var register = { request: "register", username: encodeURI(username) };
...
function registerUsername(username) {
	...
	if (username === "") {
		if ('bootbox_alert' in window) {
			bootbox_alert("Insert a username to register (e.g., pippo)");
		} else {
	...
	//if (lastUserList) {
	//	realRegisterUsername(username);
	//} else {
	//	waitRegisterUsername = username;
	//}
	var register = { request: "register", username: encodeURI(username) };
	videocall.send({ message: register });
	//waitRegisterUsername = username;
...
function doCall(username) {
	...
	if(username === "") {
		if ('bootbox_alert' in window) {
			bootbox_alert("Insert a username to call (e.g., pluto)");
		} else {
	...
	videocall.createOffer({
		...
		success: function(jsep) {
			Janus.debug("Got SDP!", jsep);
			var body = { request: "call", username: encodeURI(username) };
			videocall.send({ message: body, jsep: jsep });
		},
		error: function(error) {
			Janus.error("WebRTC error...", error);
			if ('bootbox_alert' in window) {
				bootbox_alert("WebRTC error... " + error.message);
			} else {
...
window.destroyVideoCall = function() {
	if (janus) {
		janus.destroy();
	}
}
...
function sendData() {
	...
	if(data === "") {
		if ('bootbox_alert' in window) {
			bootbox_alert('Insert a message to send on the DataChannel to your peer');
		} else {
