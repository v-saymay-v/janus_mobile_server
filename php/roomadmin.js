...
function updateServerInfo() {
	...
	$.ajax({
		...
		success: function(json) {
			if(json["janus"] !== "server_info") {
				...
				if(!prompting && !alerted) {
					alerted = true;
					bootboxalert(json["error"].reason, function() {
						promptAccessDetails();
						alerted = false;
					});
				}
		...
		error: function(XMLHttpRequest, textStatus, errorThrown) {
			...
			if(!prompting && !alerted) {
				alerted = true;
				bootboxalert("Couldn't contact the backend: is Janus down, or is the Admin/Monitor interface disabled?", function() {
					promptAccessDetails();
					alerted = false;
				});
			}
				...
				if(k === 'session_timeout') {
					...
					$('#'+k + "_button").click(function() {
						bootbox.prompt("Set the new session timeout value (in seconds, currently " + settings["session_timeout"] + ")", function(result) {
							if(isNaN(result)) {
								bootboxalert("Invalid session timeout value");
								return;
							}
					...
					$('#'+k + "_button").click(function() {
						bootbox.prompt("Set the new desired log level (0-7, currently " + settings["log_level"] + ")", function(result) {
							if(isNaN(result)) {
								bootboxalert("Invalid log level (should be [0,7])");
								return;
							}
							result = parseInt(result);
					...
					$('#'+k + "_button").click(function() {
						bootbox.prompt("Set the new desired min NACK queue (a positive integer, currently " + settings["min_nack_queue"] + ")", function(result) {
							if(isNaN(result)) {
								bootboxalert("Invalid min NACK queue (should be a positive integer)");
								return;
							}
							result = parseInt(result);
							if(result < 0) {
					...
					$('#'+k + "_button").click(function() {
						bootbox.prompt("Set the new desired no-media timer value (in seconds, currently " + settings["no_media_timer"] + ")", function(result) {
							if(isNaN(result)) {
								bootboxalert("Invalid no-media timer (should be a positive integer)");
								return;
							}
							result = parseInt(result);
							if(result < 0) {
					...
					$('#'+k + "_button").click(function() {
						bootbox.prompt("Set the new desired slowlink-threshold value (in lost packets per seconds, currently " + settings["slowlink_threshold"] + ")", function(result) {
							if(isNaN(result)) {
								bootboxalert("Invalid slowlink-threshold timer (should be a positive integer)");
								return;
							}
							result = parseInt(result);
							if(result < 0) {
...
function resetPluginRequest() {
	...
	$('#sendmsg').click(function() {
		...
		for(var i=0; i<num; i++) {
			...
			if(name === '') {
				bootboxalert("Missing name in attribute #" + (i+1));
				return;
			}
			if(message[name] !== null && message[name] !== undefined) {
				bootboxalert("Duplicate attribute '" + name + "'");
				return;
			}
			var value = $('#attrvalue'+i).val();
			if(value === '') {
				bootboxalert("Missing value in attribute #" + (i+1));
				return;
			}
			...
			if(type === "number") {
				...
				if(isNaN(value)) {
					bootboxalert("Invalid value in attribute #" + (i+1) + " (expecting a number)");
					return;
				}
				...
				if(value.toLowerCase() === "true") {
					...
					bootboxalert("Invalid value in attribute #" + (i+1) + " (expecting a boolean)");
					return;
				}
			}
	...
	$.ajax({
		...
		success: function(json) {
			...
			$('#addtoken').click(function() {
				...
				if(token === "") {
					bootboxalert("Please insert a valid token string");
					return;
				}
				var checked = $(':checked');
				if(checked.length === 0) {
					bootboxalert("Please allow the token access to at least a plugin");
					return;
				}
...
function captureTrafficRequest(start, text, folder, filename, truncate) {
	...
	$.ajax({
		...
		success: function(json) {
			if(json["janus"] !== "success") {
				console.log("Ooops: " + json["error"].code + " " + json["error"].reason);	// FIXME
				bootboxalert(json["error"].reason);
